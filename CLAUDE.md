...
### Checkout & Order Flow (UPI / COD)

Checkout steps: Address → Schedule → **Payment (UPI or COD)** → Confirmation.

- **UPI** (default): `src/components/checkout/PaymentStep.jsx` shows a per-order QR generated by the `qrcode` package. Deep link is built in `src/config/upiPayment.js` (UPI ID `9081668490@kotakbank`, account holder Momin Akbarhusen Gulamali). After paying, customer optionally enters a transaction ref and taps "I've Paid" — order is marked `paymentStatus: 'claimed'`.
- **COD**: pay on delivery, no QR shown; order is marked `paymentStatus: 'pending'`.

After payment selection, ConfirmationStep:
1. Sends WhatsApp message to admin `+91 90816 68490` with order details + UPI ref (or COD amount-to-collect)
2. Saves order to Google Sheet "Orders" tab via Apps Script (`src/config/googleSheetOrders.js`)
3. Shows 30-min cancellation countdown — cancelling fires another WhatsApp to admin

There's no Razorpay anymore (removed). Admin verifies UPI receipts manually in their bank app, then sends approval to customer via WhatsApp.
